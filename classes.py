...
def query2(list_to_sort):
    ...
    lang_list = set([i[0] for i in list_to_sort])

    for l in lang_list:
        env_dates = [i[2] for i in list(filter(lambda i: i[0] == l, list_to_sort))] # Список лет выхода каждой IDE для конкретного языка
        res_12_unsorted.append((l, min(env_dates)))
...
def query3(list_to_sort):
    ...
    env_list = set([i[0] for i in list_to_sort])

    for e in env_list:
        if 'V' == e[0]:
            # Список языков для данной среды
            e_langs = set([i[2] for i in list(filter(lambda x: x[0] == e, list_to_sort))])
            res_13[e] = e_langs
